...
export default function FichaEvoDesktop({ mode = 'create', initialData, onSave, onVoltar, pacienteSelecionado, pacienteNomeProp }) {
  ...
  // Referência para controlar dinamicamente a altura do textarea
  const textareaRef = useRef(null);

  // Função para ajustar automaticamente a altura do textarea de evolução sem scroll interno
      ...
      >
        ...
        >
          {!isView && (
            <button
              type="button"
              className="btn-acao-ficha"
              onClick={handleSubmit}
            >
              <MdSave size={20} />
              SALVAR FICHA DE EVOLUÇÃO
            </button>
          )}
