...
import { MdSave, MdDateRange, MdArrowBack, MdImage, MdKeyboardArrowDown, MdKeyboardArrowUp } from 'react-icons/md';
import { collection, onSnapshot } from 'firebase/firestore';
import { db } from './firebase';

export default function FichaEvoMobile({
  mode = 'create',
  initialData,
  onSave,
  onVoltar,
  ...
  useEffect(() => {
    const nomeFinal = dadosOrigem.nome || dadosOrigem.nomeCliente || pacienteSelecionado?.nome || pacienteNomeProp || initialData?.nomeCliente || initialData?.paciente || '';
    const nascFinal = dadosOrigem.dataNasc || dadosOrigem.dataNascimento || dadosOrigem.nascimento || initialData?.dataNasc || initialData?.dataNascimento || '';
    const telFinal = dadosOrigem.telefone || dadosOrigem.celular || dadosOrigem.fone || initialData?.telefone || initialData?.celular || '';
    const endFinal = dadosOrigem.endereco || dadosOrigem.end || initialData?.endereco || '';
    const docFinal = dadosOrigem.documento || dadosOrigem.nDocumento || dadosOrigem.numeroDocumento || dadosOrigem.cpf || dadosOrigem.rg || initialData?.documento || initialData?.nDocumento || '';
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .input-line { width: 100%; border: none; background: transparent; border-bottom: 1.5px solid #D4AF37; outline: none; height: 24px; font-size: 13px; font-weight: 500; color: #1A1A1A; margin-top: 2px; -webkit-text-fill-color: #1A1A1A; }
        input, select, textarea { color: #1A1A1A !important; -webkit-text-fill-color: #1A1A1A !important; }
      `}</style>

      <div style={{ width: '100%', maxWidth: '100%', position: 'relative', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', backgroundColor: '#f3eef8', borderRadius: '0' }}>
        <div style={{ width: '100%', height: '140px', backgroundImage: 'url("/imagens/moldura-topo-evolucao.jpeg")', backgroundRepeat: 'no-repeat', backgroundPosition: 'top center', backgroundSize: 'cover', flexShrink: 0, zIndex: 3 }} />

        <div style={{
          width: '100%', backgroundImage: 'url("/imagens/moldura-meio.jpeg")',
          ...
          <div style={{
            ...
            backgroundSize: 'contain', opacity: 0.35, pointerEvents: 'none', zIndex: 1,
          }} />
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <div style={{ flex: 1, position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
                      ...
                      {!isView && (
                        <>
                          <input id="evo-datanasc-picker" name="dataNascPicker" type="date" aria-label="Nascimento" onChange={(e) => { if (e.target.value) { const [ano, mes, dia] = e.target.value.split('-'); setDataNasc(`${dia}/${mes}/${ano}`); } }} style={{ position: 'absolute', right: '0', width: '28px', height: '18px', opacity: 0, cursor: 'pointer', zIndex: 3 }} />
                  ...
                  <div style={{ flex: 1, position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
                      ...
                      {!isView && (
                        <>
                          <input id="evo-datarealizacao-picker" name="dataRealizacaoPicker" type="date" aria-label="Realização" onChange={(e) => { if (e.target.value) { const [ano, mes, dia] = e.target.value.split('-'); setDataRealizacao(`${dia}/${mes}/${ano}`); } }} style={{ position: 'absolute', right: '0', width: '28px', height: '18px', opacity: 0, cursor: 'pointer', zIndex: 3 }} />
                          <MdDateRange size={18} color="#C8A24A" style={{ position: 'absolute', right: '2px', pointerEvents: 'none', zIndex: 2 }} />
                        </>
                      )}
                    </div>
        ...
        <div style={{
          ...
          {!isView && (
            <button type="button" onClick={handleSubmit} style={{ backgroundColor: '#D4AF37', color: '#FFFFFF', border: 'none', borderRadius: '6px', padding: '12px 20px', fontSize: '13px', fontWeight: 700, fontFamily: "'Cinzel', serif", cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', boxShadow: '0 2px 8px rgba(0,0,0,0.2)', width: '100%', maxWidth: '240px' }}>
              <MdSave size={18} /> SALVAR EVOLUÇÃO
            </button>
          )}
          {isView && initialData && (
            <button type="button" onClick={initialData.onIrParaEdicao} style={{ backgroundColor: '#D4AF37', color: '#FFFFFF', border: 'none', borderRadius: '6px', padding: '12px 20px', fontSize: '13px', fontWeight: 700, fontFamily: "'Cinzel', serif", cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', boxShadow: '0 2px 8px rgba(0,0,0,0.2)', width: '100%', maxWidth: '240px' }}>
              EDITAR EVOLUÇÃO
            </button>
          )}
          <button type="button" onClick={onVoltar || (() => window.history.back())} style={{ backgroundColor: '#D4AF37', color: '#FFFFFF', border: 'none', borderRadius: '6px', padding: '12px 20px', fontSize: '13px', fontWeight: 700, fontFamily: "'Cinzel', serif", cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', boxShadow: '0 2px 8px rgba(0,0,0,0.2)', width: '100%', maxWidth: '240px' }}>
